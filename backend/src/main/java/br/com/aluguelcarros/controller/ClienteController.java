...
public class ClienteController {
    ...
    public ResponseEntity<?> adicionarEmpregador(@PathVariable Long id, @RequestBody Empregador empregador) {
        try {
            return ResponseEntity.status(HttpStatus.CREATED).body(clienteService.adicionarEmpregador(id, empregador));
        } catch (IllegalArgumentException exception) {
            return erro(HttpStatus.BAD_REQUEST, exception.getMessage());
    ...
    @DeleteMapping("/{clienteId}/empregadores/{empregadorId}")
    public ResponseEntity<?> removerEmpregador(@PathVariable Long clienteId, @PathVariable Long empregadorId) {
        try {
            clienteService.removerEmpregador(clienteId, empregadorId);
            return ResponseEntity.noContent().build();
        } catch (IllegalArgumentException exception) {
            return erro(HttpStatus.BAD_REQUEST, exception.getMessage());
        } catch (NoSuchElementException exception) {
